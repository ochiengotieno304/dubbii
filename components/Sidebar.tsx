...
const Sidebar: React.FC<SidebarProps> = ({ isSidebarOpen, toggleSidebar }) => {
  ...
  const isActive = (path: string, searchParam?: string) => {
    ...
    if (path === '/browse' && !searchParam) {
      return currentPathMatches && !location.search.includes('type=') && !location.search.includes('sort=');
    }
  ...
  const renderNavItems = (type: 'general' | 'category' | 'personal') => NAV_ITEMS.filter(item => item.type === type).map(item => (
    ...
    >
      <item.icon className={`w-5 h-5 fill-current flex-shrink-0 ${isActive(item.path, item.search) ? 'text-gray-900' : 'text-gray-500 dark:text-gray-400 group-hover:text-secondary'}`} />
